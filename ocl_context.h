...
namespace ocl {
class context: public ocl_handle::shared_handle<cl_context> {
public:
    ...
    always_inline static std::expected<ocl::context, status> create(std::span<cl_device_id> devices) noexcept {
        ...
        return std::expected<ocl::context, status>( context );
    }
    ...
    always_inline static std::expected<ocl::context, status> create(std::span<cl_context_properties> context_properties, std::span<cl_device_id> devices) noexcept {
        ...
        return std::expected<ocl::context, status>(ocl::context{ context });
    }
    ...
    always_inline std::expected<ocl::queue, status> create_queue(cl_device_id device) noexcept {
        return queue::create(m_ptr, device);
    }
    ...
    always_inline std::expected<std::vector<ocl::queue>, status> create_queues(std::vector<cl_device_id> devices, std::vector<cl_queue_properties> properties) noexcept {
        ...
        for (auto&& device : devices) {
            queues.emplace_back(queue::create(m_ptr, device, properties).value());
        }
        return queues;
    }
    ...
    always_inline std::expected<std::vector<ocl::queue>, status> create_queues(std::vector<cl_device_id> devices) noexcept {
        std::vector<ocl::queue> queues;
        for (auto&& device : devices) {
            queues.emplace_back(queue::create(m_ptr, device).value());
        }
        return queues;
    }
    ...
    always_inline std::expected<std::vector<cl_device_id>, status> get_devices() const noexcept {
        return get_info_integral<cl_uint>(CL_CONTEXT_NUM_DEVICES).and_then([&](cl_uint num_devices)->std::expected<std::vector<cl_device_id>, status> {
            ...
            ocl::status sts{ clGetContextInfo(m_ptr, CL_CONTEXT_DEVICES, s_devices.size_bytes(), s_devices.data(), nullptr) };
    ...
    always_inline std::expected<ocl::svm<T>, status> alloc_svm(cl_svm_mem_flags flags, size_t size, cl_uint alignment = 0) noexcept {
        return ocl::svm<T>::alloc(m_ptr, flags, size, alignment);
    }

    always_inline static std::expected<buffer, status> from_gl(ocl::context &context, cl_mem_flags flags, cl_uint bufobj) {
        cl_int errcode_ret{};
        cl_mem mem{ clCreateFromGLBuffer(context.m_ptr, flags, bufobj, &errcode_ret) };

        if (errcode_ret) {
            return std::unexpected<status>(status{ errcode_ret });
        }
        return std::expected<ocl::buffer, status>(ocl::buffer{ mem });
