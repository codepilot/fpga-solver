    ...
    static MemoryMappedFile make(std::string fn_bin) {
        {
            ...
            jthread_each(xcvu3p::tiles, [&](uint64_t tile_idx, tile_reader& tile) {
                ...
                each<uint64_t, decltype(tile_wires), uint32_t>(tile_wires, [&](uint64_t tile_wire_idx, uint32_t wire_str_idx) {
                    ...
                    s_wire_idx_to_tile_idx_tile_wire_idx[wire_idx] = tile_idx_tile_wire_idx{ .tile_idx{static_cast<uint32_t>(tile_idx)}, .tile_wire_idx{static_cast<uint32_t>(tile_wire_idx)} };
                    });
                });
        }
