...
draw_wires(
    ...
    if (drawIndirect[get_global_id(0)][3] != 0) return; //dead end or success already

    ushort2 sourcePos = allSourcePos[get_global_id(0)];//s0.s01;
    ushort2 curPos = tile_to_coords(stubLocations[get_global_id(0)][0]);

    if (sourcePos[0] == curPos[0] && sourcePos[1] == curPos[1]) {
        // finished successfully
        drawIndirect[get_global_id(0)][3] = 1;
        return;
    }
    ...
    if (newPos[0] == curPos[0] && newPos[1] == curPos[1]) {
        // dead end
        drawIndirect[get_global_id(0)] = make_uint4(count + 1, 1, get_global_id(0) * ocl_counter_max, 2);
        routed[get_global_id(0)][count] = sourcePos;
        return;
    }

    drawIndirect[get_global_id(0)] = make_uint4(count + 1, 1, get_global_id(0) * ocl_counter_max, 0);
