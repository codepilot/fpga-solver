...
int main() {
	...
	program.build().value();
	auto program_il{ program.get_info_string(CL_PROGRAM_IL).value() };
	...
	for (auto &&kernel : kernels) {
		...
		auto global_work_size{ kernel.get_work_group_info_integral<std::array<size_t, 3>>(CL_KERNEL_GLOBAL_WORK_SIZE).value_or(std::array<size_t, 3>{}) };
		auto compile_work_group_size{ kernel.get_work_group_info_integral<std::array<size_t, 3>>(CL_KERNEL_COMPILE_WORK_GROUP_SIZE).value() };
		std::cout << std::format("GLOBAL_WORK_SIZE: {} {} {}\n", global_work_size.at(0), global_work_size.at(1), global_work_size.at(2));
		std::cout << std::format("WORK_GROUP_SIZE: {}\n", kernel.get_work_group_info_integral<size_t>(CL_KERNEL_WORK_GROUP_SIZE).value());
		...
		std::cout << std::format("LOCAL_MEM_SIZE: {}\n", kernel.get_work_group_info_integral<cl_ulong>(CL_KERNEL_LOCAL_MEM_SIZE).value());
		std::cout << std::format("PREFERRED_WORK_GROUP_SIZE_MULTIPLE: {}\n", kernel.get_work_group_info_integral<size_t>(CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE).value());
		std::cout << std::format("PRIVATE_MEM_SIZE: {}\n", kernel.get_work_group_info_integral<cl_ulong>(CL_KERNEL_PRIVATE_MEM_SIZE).value());
