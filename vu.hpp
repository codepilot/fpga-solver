...
namespace vk_route {
				...
				std::distance(
					...
					std::ranges::find_if(
						...
						[&, memory_type_index = 0u](const vk::MemoryType& memoryType) mutable -> bool {
							...
							return true;
							//return (memoryType.propertyFlags & needed_flags) == needed_flags;
						}
					)
				)
			);
	...
	public:
		...
		static vk::StructureChain<vk::MemoryAllocateInfo, vk::ImportMemoryHostPointerInfoEXT> make_allocate_info(vk::UniqueDevice& device, std::span<vk::MemoryHeap> memory_heaps, std::span<vk::MemoryType> memory_types, vk::MemoryRequirements2& requirements, AlignedAllocation& allocation) noexcept {
			vk::StructureChain<vk::MemoryAllocateInfo, vk::ImportMemoryHostPointerInfoEXT> allocateInfo;
			...
			std::bitset<32> allowed_types(static_cast<uint64_t>(hostPointerProperties.memoryTypeBits) & static_cast<uint64_t>(requirements.memoryRequirements.memoryTypeBits));
			vk::MemoryPropertyFlags needed_flags{ vk::MemoryPropertyFlagBits::eHostVisible };
			for (uint32_t allowed_type_idx = 0; allowed_type_idx < allowed_types.size(); allowed_type_idx++) {
				if (!allowed_types[allowed_type_idx]) continue;
				decltype(auto) memoryType{ memory_types[allowed_type_idx] };
				decltype(auto) memoryHeap{ memory_heaps[memoryType.heapIndex] };
				if (requirements.memoryRequirements.size > memoryHeap.size) {
					allowed_types.reset(allowed_type_idx);
				}
			}
#ifdef _DEBUG
		...
		HostMemoryBuffer(
			...
			allocateInfo{ make_allocate_info(device, memory_heaps, memory_types, requirements, allocation) },
			memory{ label(device, bufferLabel, device->allocateMemoryUnique(allocateInfo.get<vk::MemoryAllocateInfo>()).value) },
			buffer{ label(device, bufferLabel, device->createBufferUnique(bci.get<vk::BufferCreateInfo>()).value) }
		{
#ifdef _DEBUG
